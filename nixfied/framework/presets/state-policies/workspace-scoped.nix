{ config, lib, ... }:
{
  config.nixfied.state.policy = {
    ...
    roots.runtimeBase =
      lib.mkDefault "/tmp/nixfied-runtime/${config.nixfied.identity.projectId}/{workspaceId}/runtime";
    roots.registryRoot =
      lib.mkDefault "/tmp/nixfied-runtime/${config.nixfied.identity.projectId}/{workspaceId}/registry";
    roots.artifactsRoot =
      lib.mkDefault "/tmp/nixfied-artifacts-${config.nixfied.identity.projectId}-{workspaceId}";
  };
}
