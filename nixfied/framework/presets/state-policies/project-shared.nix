{ config, lib, ... }:
{
  config.nixfied.state.policy = {
    ...
    roots.runtimeBase = lib.mkDefault "\${NIX_BUILD_TOP:-\${XDG_CACHE_HOME:-$HOME/.cache}}/nixfied-runtime/${config.nixfied.identity.projectId}/runtime";
    roots.registryRoot = lib.mkDefault "\${NIX_BUILD_TOP:-\${XDG_CACHE_HOME:-$HOME/.cache}}/nixfied-runtime/${config.nixfied.identity.projectId}/registry";
