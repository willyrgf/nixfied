...
assert pkgs.lib.hasInfix "selected_shards=(\"''\${SHARDS[@]}\")" source;
...
assert pkgs.lib.hasInfix "run-id-active-collision-suffix-smoke" source;
assert pkgs.lib.hasInfix "selected-source-only-resolution-smoke" source;
assert pkgs.lib.hasInfix "disabled-service-no-package-resolution-smoke" source;
...
pkgs.runCommand "framework-test-coverage-contract" { } ''
  echo "OK: framework::test defaults to all shards and keeps targeted shards" > "$out"
''
