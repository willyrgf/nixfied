...
let
  ...
  runtimeEventsSource = builtins.readFile ../../nixfied/framework/runtime/runtime-events-programs.nix;
...
assert pkgs.lib.hasInfix "import ./command-runtime.nix" helpersSource;
...
assert pkgs.lib.hasInfix "import ./helpers/kernel-export-runtime.nix" runtimeEventsSource;
...
pkgs.runCommand "helpers-runtime-contract" { } ''
  echo "OK: helper shells stay thin and delegate runtime event programs and summary work to the kernel" > "$out"
''
