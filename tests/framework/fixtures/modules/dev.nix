  ...
  commands = {
    dev = {
      ...
      script = ''
        ...
        run_hook SVC_POSTGRES_INIT
        run_hook SVC_POSTGRES_CHECK_CONFIG

        POSTGRES_STARTED=0
        if nc -z 127.0.0.1 "$PGPORT" >/dev/null 2>&1; then
          echo "Skipping SVC_POSTGRES_START (port in use)" >&2
        else
          set +e
          run_hook SVC_POSTGRES_HEALTH >/dev/null 2>&1
          POSTGRES_PRE_HEALTH_RC=$?
          set -e
          if [ "$POSTGRES_PRE_HEALTH_RC" -eq 0 ]; then
            echo "SVC_POSTGRES_HEALTH should fail before start" >&2
            exit 1
          fi

          set +e
          run_hook SVC_POSTGRES_READY >/dev/null 2>&1
          POSTGRES_PRE_READY_RC=$?
          set -e
          if [ "$POSTGRES_PRE_READY_RC" -eq 0 ]; then
            echo "SVC_POSTGRES_READY should fail before start" >&2
            exit 1
          fi

          set +e
          run_hook SVC_POSTGRES_START
          POSTGRES_RC=$?
          set -e
          if [ "$POSTGRES_RC" -ne 0 ]; then
            echo "WARN: skipping SVC_POSTGRES_START checks start_failed=1" >&2
            print_log_tail "$PGDATA/postgres.log" 50
            cleanup_hook_pid SVC_POSTGRES_STOP "" "postgres"
          else
            POSTGRES_STARTED=1
          fi
        fi
        ...
        if [ "$POSTGRES_STARTED" -eq 1 ]; then
          ...
          for i in $(seq 1 60); do
            if run_hook SVC_POSTGRES_READY >/dev/null 2>&1; then
              POSTGRES_READY_OK=1
              break
            fi
            sleep 0.2
          ...
          if [ "$POSTGRES_READY_OK" -ne 1 ]; then
            echo "SVC_POSTGRES_READY should pass while running" >&2
            print_log_tail "$PGDATA/postgres.log" 50
            cleanup_hook_pid SVC_POSTGRES_STOP "" "postgres"
            exit 1
          fi

          if ! wait_hook_success SVC_POSTGRES_HEALTH 20 0.2; then
            echo "SVC_POSTGRES_HEALTH should pass while running" >&2
            print_log_tail "$PGDATA/postgres.log" 50
            cleanup_hook_pid SVC_POSTGRES_STOP "" "postgres"
            exit 1
          fi
          ...
          if ! wait_hook_failure SVC_POSTGRES_HEALTH 40 0.2; then
            echo "SVC_POSTGRES_HEALTH should fail after stop" >&2
            exit 1
          fi

          if ! wait_hook_failure SVC_POSTGRES_READY 20 0.2; then
            echo "SVC_POSTGRES_READY should fail after stop" >&2
            exit 1
          fi
        fi
        ...
        run_hook SVC_NGINX_INIT
        run_hook SVC_NGINX_SITE_PROXY example.localhost 127.0.0.1 "$BACKEND_PORT"
        run_hook SVC_NGINX_SITE_PROXY example.localhost 127.0.0.1 "$BACKEND_PORT"
        STATIC_ROOT="$NGINX_DIR/static"
        mkdir -p "$STATIC_ROOT"
        echo "static ok" > "$STATIC_ROOT/index.html"
        run_hook SVC_NGINX_SITE_STATIC static.localhost "$STATIC_ROOT"
        ...
        if run_hook SVC_POSTGRES_START >/dev/null 2>&1; then
          echo "SVC_POSTGRES_START should fail without slot/env" >&2
          exit 1
        fi
        if run_hook SVC_POSTGRES_INIT >/dev/null 2>&1; then
          echo "SVC_POSTGRES_INIT should fail without slot/env" >&2
          exit 1
        fi
        if run_hook SVC_POSTGRES_SETUP_DB >/dev/null 2>&1; then
          echo "SVC_POSTGRES_SETUP_DB should fail without slot/env" >&2
          exit 1
        fi
        if run_hook SVC_POSTGRES_HEALTH >/dev/null 2>&1; then
          echo "SVC_POSTGRES_HEALTH should fail without slot/env" >&2
          exit 1
        fi
        if run_hook SVC_POSTGRES_READY >/dev/null 2>&1; then
          echo "SVC_POSTGRES_READY should fail without slot/env" >&2
          exit 1
        fi
        if run_hook SVC_NGINX_HEALTH >/dev/null 2>&1; then
          echo "SVC_NGINX_HEALTH should fail without slot/env" >&2
          exit 1
        fi
        if run_hook SVC_NGINX_READY >/dev/null 2>&1; then
          echo "SVC_NGINX_READY should fail without slot/env" >&2
          exit 1
        fi
        ...
        if run_hook SVC_NGINX_SITE_PROXY >/dev/null 2>&1; then
          echo "SVC_NGINX_SITE_PROXY should fail without args" >&2
          exit 1
        fi
        if run_hook SVC_NGINX_SITE_STATIC >/dev/null 2>&1; then
          echo "SVC_NGINX_SITE_STATIC should fail without args" >&2
          exit 1
        fi

        if nc -z 127.0.0.1 "$HTTP_PORT" >/dev/null 2>&1 || nc -z 127.0.0.1 "$HTTPS_PORT" >/dev/null 2>&1; then
          echo "Skipping SVC_NGINX_START (port in use)" >&2
        else
          set +e
          run_hook SVC_NGINX_HEALTH >/dev/null 2>&1
          NGINX_PRE_HEALTH_RC=$?
          set -e
          if [ "$NGINX_PRE_HEALTH_RC" -eq 0 ]; then
            echo "SVC_NGINX_HEALTH should fail before start" >&2
            exit 1
          fi

          set +e
          run_hook SVC_NGINX_READY >/dev/null 2>&1
          NGINX_PRE_READY_RC=$?
          set -e
          if [ "$NGINX_PRE_READY_RC" -eq 0 ]; then
            echo "SVC_NGINX_READY should fail before start" >&2
            exit 1
          fi

          NGINX_PID=$(start_service nginx -- "$SVC_NGINX_START")
          for i in $(seq 1 20); do
          ...
          if [ "$NGINX_READY_OK" -ne 1 ]; then
            echo "SVC_NGINX_READY should pass while running" >&2
            print_log_tail "$NGINX_DIR/logs/error.log" 50
            cleanup_hook_pid SVC_NGINX_STOP "$NGINX_PID" "nginx"
            exit 1
          fi

          if ! wait_hook_success SVC_NGINX_HEALTH 20 0.2; then
            echo "SVC_NGINX_HEALTH should pass while running" >&2
            print_log_tail "$NGINX_DIR/logs/error.log" 50
            cleanup_hook_pid SVC_NGINX_STOP "$NGINX_PID" "nginx"
            exit 1
          fi

          run_hook SVC_NGINX_STOP
          for i in $(seq 1 20); do
            if [ ! -f "$NGINX_DIR/run/nginx.pid" ]; then
              break
            fi
          ...
          if ! wait_hook_failure SVC_NGINX_HEALTH 40 0.2; then
            echo "SVC_NGINX_HEALTH should fail after stop" >&2
            exit 1
          fi
          if ! wait_hook_failure SVC_NGINX_READY 20 0.2; then
            echo "SVC_NGINX_READY should fail after stop" >&2
            exit 1
          fi
          cleanup_hook_pid SVC_NGINX_STOP "$NGINX_PID" "nginx"
        fi
        ...
        run_hook SVC_MINIO_CHECK_CONFIG

        if nc -z 127.0.0.1 "$MINIOAPI_PORT" >/dev/null 2>&1 || nc -z 127.0.0.1 "$MINIOCONSOLE_PORT" >/dev/null 2>&1; then
          echo "Skipping SVC_MINIO_START (port in use)" >&2
        else
          set +e
          run_hook SVC_MINIO_HEALTH >/dev/null 2>&1
          MINIO_PRE_HEALTH_RC=$?
          set -e
          if [ "$MINIO_PRE_HEALTH_RC" -eq 0 ]; then
            echo "SVC_MINIO_HEALTH should fail before start" >&2
            exit 1
          fi

          set +e
          run_hook SVC_MINIO_READY >/dev/null 2>&1
          MINIO_PRE_READY_RC=$?
          set -e
          if [ "$MINIO_PRE_READY_RC" -eq 0 ]; then
            echo "SVC_MINIO_READY should fail before start" >&2
            exit 1
          fi

          MINIO_PID=""
          MINIO_PID=$(start_service minio -- "$SVC_MINIO_START")
          MINIO_READY_OK=0
          for i in $(seq 1 120); do
            if run_hook SVC_MINIO_READY >/dev/null 2>&1; then
              MINIO_READY_OK=1
              break
            fi
            sleep 0.2
          ...
          if [ "$MINIO_READY_OK" -ne 1 ]; then
            ...
            cleanup_hook_pid SVC_MINIO_STOP "$MINIO_PID" "minio"
            exit 1
          fi

          if ! wait_hook_success SVC_MINIO_HEALTH 30 0.2; then
            echo "SVC_MINIO_HEALTH should pass while running" >&2
            print_log_tail "$MINIO_DIR/logs/minio.log" 50
            cleanup_hook_pid SVC_MINIO_STOP "$MINIO_PID" "minio"
            exit 1
          fi

          run_hook SVC_MINIO_BUCKET_LIST >/dev/null
          run_hook SVC_MINIO_STOP

          if ! wait_hook_failure SVC_MINIO_HEALTH 40 0.2; then
            echo "SVC_MINIO_HEALTH should fail after stop" >&2
            exit 1
          fi

          if ! wait_hook_failure SVC_MINIO_READY 20 0.2; then
            echo "SVC_MINIO_READY should fail after stop" >&2
            exit 1
          fi
        ...
        if nc -z 127.0.0.1 "$RETHHTTP_PORT" >/dev/null 2>&1 || nc -z 127.0.0.1 "$RETHWS_PORT" >/dev/null 2>&1 || nc -z 127.0.0.1 "$RETHAUTH_PORT" >/dev/null 2>&1; then
          echo "Skipping SVC_RETH_START (port in use)" >&2
          if run_hook SVC_RETH_HEALTH >/dev/null 2>&1; then
            RETH_AVAILABLE=1
          fi
        else
          set +e
          run_hook SVC_RETH_HEALTH >/dev/null 2>&1
          RETH_PRE_HEALTH_RC=$?
          set -e
          if [ "$RETH_PRE_HEALTH_RC" -eq 0 ]; then
            echo "SVC_RETH_HEALTH should fail before start" >&2
            exit 1
          fi

          set +e
          run_hook SVC_RETH_READY >/dev/null 2>&1
          RETH_PRE_READY_RC=$?
          set -e
          if [ "$RETH_PRE_READY_RC" -eq 0 ]; then
            echo "SVC_RETH_READY should fail before start" >&2
            exit 1
          fi

          RETH_PID=$(start_service reth -- "$SVC_RETH_START")
          RETH_READY_OK=0
          for i in $(seq 1 120); do
          ...
          if [ "$RETH_READY_OK" -ne 1 ]; then
            ...
            cleanup_hook_pid SVC_RETH_STOP "$RETH_PID" "reth"
            exit 1
          fi
          ...
          if [ "$RETH_HEALTH_OK" -ne 1 ]; then
            echo "SVC_RETH_HEALTH should pass while running" >&2
            print_log_tail "$RETH_DIR/logs/reth.log" 50
            cleanup_hook_pid SVC_RETH_STOP "$RETH_PID" "reth"
            exit 1
          fi
          ...
          if [ "$RETH_READY_STABLE" -ne 1 ]; then
            echo "SVC_RETH_READY should pass while running" >&2
            print_log_tail "$RETH_DIR/logs/reth.log" 50
            cleanup_hook_pid SVC_RETH_STOP "$RETH_PID" "reth"
            exit 1
          fi
        ...
        if [ "$RETH_AVAILABLE" -ne 1 ]; then
          echo "Skipping SVC_HELIOS_START (reth unavailable)" >&2
        else
          ...
          HELIOS_CHECK_OUTPUT="$(run_hook SVC_HELIOS_CHECK_CONFIG 2>&1)" || {
          ...
          if [ "$HELIOS_NETWORK_VALUE" = "local" ]; then
            echo "SKIP: SVC_HELIOS_START skipped network=local requires beacon consensus endpoint in this harness" >&2
          elif nc -z 127.0.0.1 "$HELIOSRPC_PORT" >/dev/null 2>&1; then
            echo "Skipping SVC_HELIOS_START (port in use)" >&2
          else
            set +e
            run_hook SVC_HELIOS_HEALTH >/dev/null 2>&1
            HELIOS_PRE_HEALTH_RC=$?
            set -e
            if [ "$HELIOS_PRE_HEALTH_RC" -eq 0 ]; then
              echo "SVC_HELIOS_HEALTH should fail before start" >&2
              exit 1
            fi

            set +e
            run_hook SVC_HELIOS_READY >/dev/null 2>&1
            HELIOS_PRE_READY_RC=$?
            set -e
            if [ "$HELIOS_PRE_READY_RC" -eq 0 ]; then
              echo "SVC_HELIOS_READY should fail before start" >&2
              exit 1
            fi

            HELIOS_PID=""
            HELIOS_PID=$(start_service helios -- "$SVC_HELIOS_START")
            export HELIOS_READY_TIMEOUT_SECS="2"
            HELIOS_READY_OK=0
            for i in $(seq 1 120); do
              if run_hook SVC_HELIOS_READY >/dev/null 2>&1; then
                HELIOS_READY_OK=1
                break
              fi
              sleep 0.2
            ...
            if [ "$HELIOS_READY_OK" -ne 1 ]; then
              ...
              cleanup_hook_pid SVC_HELIOS_STOP "$HELIOS_PID" "helios"
              exit 1
            fi

            if ! wait_hook_success SVC_HELIOS_HEALTH 20 0.2; then
              echo "SVC_HELIOS_HEALTH should pass while running" >&2
              print_log_tail "$HELIOS_DIR/logs/helios.log" 50
              cleanup_hook_pid SVC_HELIOS_STOP "$HELIOS_PID" "helios"
              exit 1
            fi
            ...
            if [ "$HELIOS_DOWN" -ne 1 ]; then
              echo "SVC_HELIOS_HEALTH should fail after stop" >&2
              exit 1
            fi

            export HELIOS_READY_TIMEOUT_SECS="0"
            if ! wait_hook_failure SVC_HELIOS_READY 20 0.2; then
              echo "SVC_HELIOS_READY should fail after stop" >&2
              exit 1
            fi
        ...
        if [ "$RETH_STARTED" -eq 1 ]; then
          ...
          if [ "$RETH_DOWN" -ne 1 ]; then
            echo "SVC_RETH_HEALTH should fail after stop" >&2
            exit 1
          fi

          if ! wait_hook_failure SVC_RETH_READY 20 0.2; then
            echo "SVC_RETH_READY should fail after stop" >&2
            exit 1
          fi
