...
assert !(pkgs.lib.hasInfix "slot_acquired) EVENT_STATE=\"busy\"" source);
assert !(pkgs.lib.hasInfix "infer_owner_scope()" source);
assert !(pkgs.lib.hasInfix "infer_discovery_scope()" source);
assert !(pkgs.lib.hasInfix "validate_policy_matrix()" source);
assert !(pkgs.lib.hasInfix "nixfied_policy_infer_reuse_policy" source);
