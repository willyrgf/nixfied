...
pkgs.runCommand "workflow-parallel-blocked-smoke" { } ''
  ...
  first_action="$("$KERNEL" workflow parallel-next "$STATE_FILE")"
  ...
  "$KERNEL" workflow parallel-transition "$STATE_FILE" a canceled "" blocked "" "" > /dev/null

  second_action="$("$KERNEL" workflow parallel-next "$STATE_FILE")"
  ...
  "$KERNEL" workflow parallel-transition "$STATE_FILE" b canceled "" blocked "" "" > /dev/null

  final_action="$("$KERNEL" workflow parallel-next "$STATE_FILE")"
