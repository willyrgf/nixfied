...
pkgs.runCommand "workflow-mode-derived-smoke" { } ''
  ...
  NIXFIED_PARALLEL_SMOKE=1 "$ORCH" run-workflow workflow.test.parallel.smoke --mode parallel.smoke --run-id-file "$TMPDIR/mode-complex.run-id" --summary-file "$TMPDIR/mode-complex.summary.json" --summary > "$TMPDIR/mode-complex.out" 2>&1
  require_non_empty "$(read_trimmed_file "$TMPDIR/mode-complex.run-id")" "mode-complex run id"
  require_file "$TMPDIR/mode-complex.summary.json"
