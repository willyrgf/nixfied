...
wrapper and does not depend on the process runtime. Runtime-backed project apps
use the release `nixfied-runtime` package; inspect their command relationships with
`nix run .#docs -- api app project/run`. The debug runtime and test child are
private framework-check inputs, not adopter-facing package outputs.
