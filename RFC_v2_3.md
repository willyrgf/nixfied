...
> **Invariant ABI-1:** A model is admitted only when its `runtimeAbi` and `toolchainId` exactly match the runtime. No cross-version compatibility is promised.
...
The Rust runtime understands generic primitives, not project-specific adapters.
...
- **`ServiceSpec`** - long-lived foreground process defined by `ExecSpec`, endpoint set, readiness, health, stop policy, state roots, logs, lifecycle policy, containment requirement.
...
Nix-side adapters are libraries that generate these primitives. For example, a Postgres module may produce one `ServiceSpec`, endpoint declarations, probes, state policy, tasks, and docs metadata. The runtime should not have Postgres-specific logic.
...
> **Invariant RUNTIME-GENERIC-1:** The runtime executes generic model primitives only. Concrete adapters are Nix-side model generators unless a later milestone explicitly introduces a runtime adapter protocol.
...
- **Service runtime hash:** exact identity of the service runtime semantics: exec paths, args, env shape, endpoints, probes, state epoch, cleanup policy, target identity, and runtime capabilities. A changed hash means "different instance", not "compatible upgrade".
- **Service instance:** a reusable running service identified by `serviceInstanceId = hash(serviceAddress, serviceRuntimeHash, logicalPortSet, targetIdentity)`.
...
- `modelHash` - runtime-computed hash of admitted raw model bytes.
...
Service identity is split into stable address and exact runtime identity:

```text
serviceAddress     = hash(projectId, environment, slot, serviceName)
serviceRuntimeHash = hash(service runtime semantics)
serviceInstanceId  = hash(serviceAddress, serviceRuntimeHash, logicalPortSet, target)
```

`serviceRuntimeHash` covers exec paths, args, environment shape, endpoint policy, readiness and health definitions, state epoch, cleanup policy, containment requirements, target identity, and runtime capabilities.
...
- **RUNTIME-GENERIC-1:** The runtime executes generic model primitives only.
...
- secret values are rejected or redacted before persistence.
...
- **Workflow:** workflows express service requirements, task dependencies, cancellation, artifacts, summaries, and cleanup policy.
- **Secrets:** models contain secret refs only; persistent runtime output is redacted before write.
...
- Service reuse could blur incompatible runtime configs. Hence exact `serviceRuntimeHash`.
- Lease/refcount semantics could depend on a daemon that does not exist. Hence run/service/borrower lease split.
- Process containment differs by platform. Hence capability-gated containment and foreground-only M0 services.
