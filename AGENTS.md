...
- **Runtime output projection:** `run` defaults to `summary`: human progress,
  ...
  `durationMs` and evidence paths); `--both` emits both projections explicitly.
  Child stdout/stderr remains in redacted log files and is not replayed inline.
  There is no `logs` control command in the current surface.
